...
if(class(model)[1]=="lgb.Booster"){
  ...
  trees<-lgb.trees[,`:=`(Tree=tree_index,
                         ...
                         Split=threshold,
                         Missing=NA,
                         Quality=ifelse(is.na(split_feature),leaf_value,split_gain),
                         Cover=count)]
  ...
  for (tree in treeList){
    ...
    for(i in 1:num_nodes){
      if(is.na(tree[i,node_parent])==FALSE){
        ...
        }else{tree[Node==tree[i,node_parent] ,No:=paste(tree[i,Tree],tree[i,Node], sep="-")]}
      }
      if(is.na(tree[i,leaf_parent])==FALSE){
        ...
        }else{tree[Node==tree[i,leaf_parent] ,No:=paste(tree[i,Tree],tree[i,Node], sep="-")]}
      }
    }
  }
